...
const SummaryPage = ({setStep}: AppPassingProps) => {
  ...
  const hasOptions = (orderDatas?.options.size ?? 0) > 0;
  ...
  if (hasOptions) {
    ...
    optionsRender = (
      <>
        <h2>옵션: {orderDatas?.totals.options ?? 0}</h2>
        <ul>
          {optionList}
        </ul>
      </>
  ...
  return (
    <div>
      <h1>주문 확인</h1>
      <h2>여행 상품: {orderDatas?.totals.products ?? 0}</h2>
      <ul>
        {productList}
      </ul>
      {optionsRender}
